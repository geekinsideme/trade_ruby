...
class LongEntryRule < Rule
  ...
  def golden_cross(trade, stocks, issue)
...
class ShortEntryRule < Rule
  ...
  def dead_cross(trade, stocks, issue)
...
class LongExitRule < Rule
  def by_execution_price(trade, stocks, issue)
    @disc = "約定金額の :limit_percent %上で指値、:stop_percent %下で逆指値"
    { exit_type: :limit_and_stop, exit_expiry: trade.execution_date+@p[:day], exit_limit_price: trade.execution_price * (1.0+@p[:limit_percent]/100.0), exit_stop_price: trade.execution_price * (1.0-@p[:stop_percent]/100.0) }
  end
end
